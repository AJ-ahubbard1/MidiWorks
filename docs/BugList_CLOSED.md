# CLOSED Bug List
## Look at all of these squashed bugs!
...
### #41 - Solo track during playback causes stuck notes (missing Note Off)
**Status:** Fixed
**Priority:** High
**Found:** 2026-01-18
**Fixed:** 2026-01-18
...
**Description:**
When playing a multi-track song and hitting solo on a track, notes from other tracks got stuck "on" because their Note Off messages were never sent. This caused sustained notes that continued playing indefinitely until manually stopped.

**Root Cause:**
When solo was activated or deactivated during playback, the solo logic immediately changed which channels should play through `ShouldChannelPlay()`, but any notes currently playing (Note On sent but Note Off not yet sent) on affected channels never received their corresponding Note Off message, leaving them in a sustained state.

**Problem Scenarios:**

**Scenario 1: Solo Enabled**
1. Channels A, B, C playing during playback
2. User solos channel A
3. `ShouldChannelPlay(B)` and `ShouldChannelPlay(C)` return `false`
4. New notes won't play on B and C ✓
5. **Problem**: Notes already playing on B and C never receive Note Off ✗

**Scenario 2: Solo Disabled (Edge Case)**
1. Channels A and B are both soloed and playing
2. User unsolos channel A (B still soloed)
3. `ShouldChannelPlay(A)` returns `false` (because `SolosFound()` is still true)
4. New notes won't play on A ✓
5. **Problem**: Notes already playing on A never receive Note Off ✗

**Solution:**

**1. Added `SilenceNonSoloedChannels()` method** (SoundBank.h:94, SoundBank.cpp:142-154)
```cpp
...
This method sends `AllNotesOff` MIDI CC to every channel that doesn't have solo enabled, immediately stopping all active notes on those channels.

**2. Updated Solo Button Handler** (ChannelControls.h:263-276)
```cpp
void OnSoloToggled(wxCommandEvent& event)
{
    ...
    if (mChannel.solo)
    {
        // Solo enabled: silence all non-soloed channels
...
**Key Logic:**
- **When solo is enabled**: Silence all non-soloed channels immediately before solo filtering takes effect
- **When solo is disabled**: Check if other channels are still soloed; if so, silence this channel (it's now excluded from the solo group)
- **When last solo is disabled**: No silencing needed - normal playback resumes for all non-muted channels

**Files Modified:**
- `src/AppModel/SoundBank/SoundBank.h` - Added `SilenceNonSoloedChannels()` declaration
- `src/AppModel/SoundBank/SoundBank.cpp` - Implemented `SilenceNonSoloedChannels()` method
- `src/Panels/ChannelControls.h` - Updated `OnSoloToggled()` to handle both solo enable/disable cases

**Benefits:**

**1. Prevents Stuck Notes** ✓
- All notes properly stopped when channels are excluded from playback
- Clean solo transitions with no hanging notes
- Professional DAW behavior

**2. Handles All Edge Cases** ✓
- Single channel solo: Works correctly
- Multiple channels soloed: Works correctly
- Unsolo one channel while others soloed: Works correctly (edge case fixed)
- Unsolo last channel: Works correctly (no unnecessary silencing)

**3. Uses Standard MIDI Protocol** ✓
- `AllNotesOff` (CC 123) is the standard MIDI message for stopping all notes
- Supported by all MIDI devices and synthesizers
- More reliable than trying to track individual active notes

**How It Works:**

**Example 1: Solo Channel A**
```
Before: A, B, C all playing
User clicks Solo on A
├─ mChannel.solo = true
├─ SilenceNonSoloedChannels() called
│  ├─ Send AllNotesOff(B)  ✓
│  └─ Send AllNotesOff(C)  ✓
└─ ShouldChannelPlay(B/C) returns false (new notes filtered)
Result: Only A continues playing ✓
```

**Example 2: Unsolo A while B is still soloed**
```
Before: A and B soloed
User clicks Solo on A (to unsolo it)
├─ wasSoloed = true (saved previous state)
├─ mChannel.solo = false
├─ SolosFound() returns true (B still soloed)
├─ Send AllNotesOff(A)  ✓ (edge case handled)
└─ ShouldChannelPlay(A) returns false
Result: Only B continues playing ✓
```

**Design Notes:**

**Why use `AllNotesOff` instead of tracking active notes?**
- **Simpler**: No need to maintain per-channel active note state
- **Reliable**: MIDI standard ensures all synthesizers support this
- **Complete**: Handles edge cases like stuck notes from external MIDI
- **Efficient**: Single MIDI message per channel vs tracking hundreds of notes

**Why check `SolosFound()` in the unsolo case?**
- If no other channels are soloed, we're returning to normal playback
- Non-soloed channels will naturally resume from current playhead position
- Only need to silence when channel is being excluded from an active solo group

**Related Fixes:**
The mute button already had similar logic (ChannelControls.h:257-260):
```cpp
...
This fix extends the same pattern to solo operations, ensuring consistent behavior across all channel routing changes.
...
**Description:**
Component classes had inconsistent documentation standards. Some classes like UndoRedoManager had excellent comprehensive documentation with responsibilities and usage examples, while others like AppModel, Transport, and NoteEditor had minimal or no class-level documentation.

**Issues:**
1. **Inconsistent standards** - New contributors didn't know which style to follow
2. **Harder to understand** - Complex classes were undocumented
3. **Knowledge gaps** - Important design decisions weren't captured in code
4. **No clear guidelines** - No reference document for documentation format

**Solution:**

**1. Created Commenting Style Guide** (docs/CommentingStyle.md)

Comprehensive style guide defining:
- **File Comments**: Use `//` for filename at top of each file
- **Class Documentation**: Use `///` for IntelliSense tooltips with:
  - Brief description
  - Responsibilities (bullet list)
  - Usage example
- **Method Grouping**: Use `//` for section headers (e.g., "// Channel Management")
- **Method Documentation**: Use `///` for method descriptions (enables IntelliSense)
- **@param and @return**: Only include when meaning isn't obvious from method name
...
**2. Applied Style Guide Consistently Across Codebase**

Updated all component classes to follow the style guide:
- **AppModel** - Added class documentation with update loop description
- **Transport** - Documented state machine and tick/time conversion
- **SoundBank** - Documented channel management and solo/mute logic
- **TrackSet** - Documented playback scheduling
- **RecordingSession** - Documented loop recording and active note tracking
- **NoteEditor** - Documented command creation pattern and preview system
- **ProjectManager** - Documented save/load and dirty state tracking
- All other component classes standardized

**3. Code Organization Guidelines**

The style guide also establishes:
- **Single-line methods**: Keep in header file
- **Method visibility**: Move unused public methods to private
- **Definition order**: Keep .cpp order matching .h declarations
- **Small classes**: Don't require separate .cpp files for simple inline methods

**Files Created:**
- `docs/CommentingStyle.md` - Complete commenting and documentation style guide

**Files Modified:**
- All major component class headers updated with consistent documentation
- Method comments standardized across codebase
- Section headers added to group related methods

**Benefits:**

**1. Clear Standards** ✓
- All developers know exactly how to document code
- Consistent style across entire codebase
- Easy reference document for new contributors

**2. Better IntelliSense** ✓
- `///` comments enable IDE tooltips
- Developers get documentation while coding
- Reduces need to constantly reference header files

**3. Improved Maintainability** ✓
- Class responsibilities clearly documented
- Design patterns explained in code
- Usage examples prevent misuse

**4. Easier Onboarding** ✓
- New contributors can quickly understand class purposes
- Design decisions captured in comments
- Reduces learning curve for codebase

**5. Self-Documenting Code** ✓
- Code explains itself through structured comments
...
| Aspect | Before | After |
|--------|--------|-------|
| Class documentation | Inconsistent | ✓ Standardized |
| Method descriptions | Missing/varied | ✓ Consistent |
| Usage examples | Rare | ✓ All major classes |
| Responsibilities | Implicit | ✓ Explicit |
| IntelliSense support | Partial | ✓ Comprehensive |
| Style guide | None | ✓ docs/CommentingStyle.md |

**Key Guidelines from CommentingStyle.md:**

**When to include @param:**
- ✓ Include when meaning isn't obvious: `@param checkRecord If true, also requires record to be enabled`
- ✗ Skip when obvious: `SetPreviewVelocity(ubyte velocity)` - no @param needed

**When to include @return:**
- ✓ Include when return value needs explanation: `@return Column index if enabled, -1 otherwise`
- ✗ Skip when obvious: `bool IsPlaying() const` - no @return needed
...
**Example - After:**
```cpp
/// Transport manages playback state and timing.
///
/// Responsibilities:
/// - Track playback state (stopped, playing, recording, etc.)
/// - Manage tick position and tempo
...
- See `docs/CommentingStyle.md` for complete style guide
- See `docs/CLAUDE.md` for architecture overview
- All classes now follow consistent commenting standard
