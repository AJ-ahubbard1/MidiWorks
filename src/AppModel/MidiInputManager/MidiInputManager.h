...
public:
	...
	void SetLogCallback(MidiLogCallback callback) { mLogCallback = callback; }

	/// Get current log callback
	/// Used internally by PollAndNotify to log events
	/// @return Current log callback (may be null)
	const MidiLogCallback& GetLogCallback() const { return mLogCallback; }
